...
class Board:
    ...
    def __init__(self):
        ...
        self.num_nums_placed = sum(1 for row in self.rows for cell in row if not isinstance(cell, list))
    ...
    def find_most_constrained_cell(self) -> Tuple[int, int]:
        ...
        most_constrained_cell = (-1, -1)

        for row_idx, row in enumerate(self.rows):
            for col_idx, cell in enumerate(row):
                if isinstance(cell, list):  # Only check unassigned cells (still lists)
                    num_possibilities = len(cell)
                    if num_possibilities < min_possibilities:
                        min_possibilities = num_possibilities
                        most_constrained_cell = (row_idx, col_idx)

        return most_constrained_cell


    def failure_test(self) -> bool:
        """check if the puzzle has failed due to any cell having no possibilities"""
    ...
    def update(self, row: int, column: int, assignment: int) -> None:
        """Assigns the given value to the cell and updates constraints on related cells."""
        if isinstance(self.rows[row][column], list):  # Check if the cell was previously a list of possibilities
            self.num_nums_placed += 1  # Increment placed numbers count

        # Assign the value to the cell
        self.rows[row][column] = assignment

        # Update the row
        for c in range(len(self.rows[row])):
            if isinstance(self.rows[row][c], list):
                if assignment in self.rows[row][c]:
                    self.rows[row][c].remove(assignment)

        # Update the column
        for r in range(len(self.rows)):
            if isinstance(self.rows[r][column], list):
                if assignment in self.rows[r][column]:
                    self.rows[r][column].remove(assignment)

        # Update the subgrid
        subgrid_size = int(len(self.rows) ** 0.5)
        start_row = (row // subgrid_size) * subgrid_size
        start_col = (column // subgrid_size) * subgrid_size
        for r in range(start_row, start_row + subgrid_size):
            for c in range(start_col, start_col + subgrid_size):
                if isinstance(self.rows[r][c], list):
                    if assignment in self.rows[r][c]:
                        self.rows[r][c].remove(assignment)
...
def DFS(state: Board) -> Board:
    ...
    Returns:
        Either a solved Board or None if no solution is found.
    """
    if state.goal_test():
        return state
    ...
    for value in current_state.rows[row][col]:
...
def BFS(state: Board) -> Board:
    """Performs a breadth-first search to solve the Sudoku puzzle.
    Args:
        state - an instance of the Board class to solve.
    Returns:
        Either a solved Board or None if no solution is found.
